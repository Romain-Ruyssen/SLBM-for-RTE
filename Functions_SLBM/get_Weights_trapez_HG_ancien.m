function [Weights,angles] = get_Weights_trapez_HG_ancien(Nb_dir,directions,g)
...
% Calcul des poids:
for i_m = 1:Nb_dir
    i_m_perm = I_permut(i_m,1);
    thetha_m = angles_tot_sort(i_m_perm);
    for i_mp = 1:Nb_dir
        i_mp_perm = I_permut(i_mp,1);
        thetha_mp = angles_tot_sort(i_mp_perm);
        if i_mp_perm == 1
            theta_mp_m1 = angles_tot_sort(end,1) - 2*pi;
            theta_mp_p1 = angles_tot_sort(2,1);
        elseif i_mp_perm == Nb_dir
            theta_mp_m1 = angles_tot_sort(Nb_dir-1,1);
            theta_mp_p1 = 2*pi + angles_tot_sort(1,1);
        else
            theta_mp_m1 = angles_tot_sort(i_mp_perm-1,1);
            theta_mp_p1 = angles_tot_sort(i_mp_perm+1,1);
        end
        ...
        f_HG_cons(theta_int_a)
        f_HG_cons(theta_int_b)
%         Weights(i_m,i_mp) = Delta_theta_tot*(0.5*f_HG_cons(theta_int_a) + 0.5*f_HG_cons(theta_int_b));    
